...
export default function EditServicePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([servicesData, resourcesData]) => {
        ...
        setNameEs(svc.name.es)
        setNameEn(svc.name.en)
        setDescEs(svc.description?.es ?? '')
        setDescEn(svc.description?.en ?? '')
        setType(svc.type)
        setResourceId(svc.resourceId ?? '')
        setPricingModel(svc.pricingModel)
        setPricePen(svc.pricePen)
        setMaxSpots(svc.maxSpots)
        setDurationMinutes(svc.durationMinutes)
        setIsAddon(svc.isAddon)
